...
import { Loader2, ChevronLeft, ChevronRight, Plus, Pencil, Trash2, Edit, Check } from 'lucide-react'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
import { PurchaseOptionsEditor } from '@/components/admin/purchase-options-editor'
import {
  Dialog,
  DialogClose,
  DialogContent,
...
export default function ProductsTable() {
  ...
  const handleFileChange = (file: File) => {
    ...
    // Show local preview immediately (no server round-trip)
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        setPreviewImageUrl(compressedBase64)
        ...
        fetch(compressedBase64)
          ...
          .then(data => {
            if (data.url) {
              setProductFormData((prev) => ({ ...prev, imageUrl: data.url }))
  ...
  const handleTogglePublish = (product: ProductRow, value: boolean) => {
    startTransition(async () => {
      const res = await fetch('/api/admin/products', {
        ...
        credentials: 'include',
        body: JSON.stringify({ typeId: product.typeId, isPublished: value }),
      })
  ...
  const handleUpdateBadge = (product: ProductRow, badge: 'hot_sale' | 'recommended' | null) => {
    startTransition(async () => {
      const res = await fetch('/api/admin/products', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ typeId: product.typeId, badge }),
      })
  ...
  const handleSavePrice = (product: ProductRow) => {
    ...
    startSavingTransition(async () => {
      const res = await fetch('/api/admin/products', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ typeId: product.typeId, price: numeric }),
      })
  ...
  const handleSavePriceVip = (product: ProductRow) => {
    ...
    startSavingTransition(async () => {
      const res = await fetch('/api/admin/products', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ typeId: product.typeId, priceVip: numeric }),
      })
  ...
  const handleSaveCostPrice = (product: ProductRow) => {
    ...
    startSavingTransition(async () => {
      const res = await fetch('/api/admin/products', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ typeId: product.typeId, costPrice: numeric }),
      })
  ...
  const handleSavePriceWalkin = (product: ProductRow) => {
    ...
    startSavingTransition(async () => {
      const res = await fetch('/api/admin/products', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ typeId: product.typeId, priceWalkin: numeric }),
      })
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          ...
                          if (draft !== current && savingCostPriceId !== product.id) handleSaveCostPrice(product)
                        }
                      }}
                      onBlur={() => {
                        const draft = costPriceDrafts[product.id]?.trim() ?? ''
                        const current = product.costPrice != null ? String(product.costPrice) : ''
                        if (draft !== current && savingCostPriceId !== product.id) {
                          handleSaveCostPrice(product)
                        }
                      }}
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          ...
                          if (draft !== current && savingPriceId !== product.id) handleSavePrice(product)
                        }
                      }}
                      onBlur={() => {
                        const draft = priceDrafts[product.id]?.trim() ?? ''
                        const current = product.price != null ? String(product.price) : ''
                        if (draft !== current && savingPriceId !== product.id) {
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          ...
                          if (draft !== current && savingPriceVipId !== product.id) handleSavePriceVip(product)
                        }
                      }}
                      onBlur={() => {
                        const draft = priceVipDrafts[product.id]?.trim() ?? ''
                        const current = product.priceVip != null ? String(product.priceVip) : ''
                        if (draft !== current && savingPriceVipId !== product.id) {
                          handleSavePriceVip(product)
                        }
                      }}
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          ...
                          if (draft !== current && savingPriceWalkinId !== product.id) handleSavePriceWalkin(product)
                        }
                      }}
                      onBlur={() => {
                        const draft = priceWalkinDrafts[product.id]?.trim() ?? ''
                        const current = product.priceWalkin != null ? String(product.priceWalkin) : ''
                        if (draft !== current && savingPriceWalkinId !== product.id) {
                          handleSavePriceWalkin(product)
                        }
                      }}
                  ...
                  try {
                    const res = await fetch('/api/admin/products', {
                      ...
                      body: JSON.stringify({
                        ...
                        stock: null, // สต็อกจะคำนวณจาก accountData อัตโนมัติ
                        categoryId: productFormData.categoryId && productFormData.categoryId !== "none" ? productFormData.categoryId : null,
                      ...
                      try {
                        const res = await fetch('/api/admin/products', {
                          ...
                          body: JSON.stringify({
                            ...
                            costPrice: productFormData.costPrice !== "" ? parseFloat(productFormData.costPrice) : null,
                            priceWalkin: productFormData.priceWalkin !== "" ? parseFloat(productFormData.priceWalkin) : null,
                            // ไม่ต้องส่ง stock เพราะจะคำนวณจาก accountData อัตโนมัติ
                            categoryId: productFormData.categoryId && productFormData.categoryId !== "none" ? productFormData.categoryId : null,
                            isPublished: productFormData.isPublished,
                            badge: productFormData.badge || null,
                          }),
