...
/**
 * The AppByMari catalog bridge is deployed independently from the database.
 * Keep the additive bootstrap idempotent so an older production database can
 * serve the Admin integration without requiring the Plesk command runner to
 * inherit the app process environment.
 */
export function ensureAppByMariSchema(): Promise<void> {
