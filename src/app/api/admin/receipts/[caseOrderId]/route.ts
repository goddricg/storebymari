...
import { getCashReceiptForAdmin } from "@/lib/receipts/repository";
...
export async function GET(
  ...
  const receipt = await getCashReceiptForAdmin(caseOrderId);
  if (!receipt) {
  ...
  try {
    ...
    console.error("Admin receipt PDF generation failed:", error);
